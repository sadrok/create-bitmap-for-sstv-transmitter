...
CHARACTER_WIDTH = 8
CHARACTER_HEIGHT = 11
MAX_LINE_LENGTH = max(len(line) for line in STATIC_LINES)
BITMAP_WIDTH = CHARACTER_WIDTH * MAX_LINE_LENGTH
BITMAP_HEIGHT = CHARACTER_HEIGHT * len(STATIC_LINES)


if __name__ == '__main__':
    bytemap = [0x00 for _ in range(len(STATIC_LINES) * CHARACTER_HEIGHT)
               for _ in range(MAX_LINE_LENGTH)]

    for line, line_text in enumerate(STATIC_LINES):
